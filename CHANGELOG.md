...
## 2026-03-22
- **T-1308**: Multi-AI War Room — `start_war_room` tool convenes 4 specialist AI panelists (Technical Analyst, Macro Strategist, Quant Analyst, Risk Manager) to debate a trading asset sequentially; each receives curated market data for their specialty (technicals/regime/confluence for Technical, macro/sentiment for Macro, quant/on-chain for Quant, ATR/risk for Risk Manager); Risk Manager speaks last with final risk verdict; consensus direction + confidence + dissent count computed from all stances; returns new `WarRoomBlock` (4-panelist debate with data citations, key points, expandable full analysis, model attribution) + `MetricCard` (consensus direction, confidence, dissenters) + `TradeSetupBlock` when consensus confidence ≥ 6; new `WarRoomBlock.svelte` renderer + `ContentBlockRenderer` case; 5-min cache; 40 tests (2491 total)
